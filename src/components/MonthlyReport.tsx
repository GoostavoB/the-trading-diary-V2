...
import { Calendar, Download, TrendingUp, TrendingDown, Target, DollarSign, Activity, Award } from 'lucide-react';
        ...
        <div className="space-y-6">
          ...
          <Card className="p-4 bg-gradient-to-br from-primary/10 to-transparent border-primary/30">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              ...
              {winRate >= 70 ? (
                <p>🎯 Outstanding {winRate.toFixed(1)}% win rate! You're trading at an elite level.</p>
              ) : winRate >= 50 ? (
                <p>👍 Solid {winRate.toFixed(1)}% win rate. Focus on letting winners run to boost profits.</p>
              ) : (
                <p>📉 {winRate.toFixed(1)}% win rate needs improvement. Review your entry criteria and risk management.</p>
              )}
              
              {profitFactor >= 2 ? (
                <p>💪 Excellent {profitFactor.toFixed(2)} profit factor. Your winners significantly outweigh your losers.</p>
              ) : (
                <p>⚖️ Work on improving your profit factor of {profitFactor.toFixed(2)}. Cut losses faster and let winners run.</p>
              )}
            </div>
          </Card>
        </div>
