...
export const MonthlyReport = ({ trades }: MonthlyReportProps) => {
  ...
  const bestTrade = monthTrades.length > 0
    ? monthTrades.reduce((best, t) => (t.profit_loss || 0) > (best.profit_loss || 0) ? t : best)
    : null;

  const worstTrade = monthTrades.length > 0
    ? monthTrades.reduce((worst, t) => (t.profit_loss || 0) < (worst.profit_loss || 0) ? t : worst)
    : null;
  ...
  monthTrades.forEach(trade => {
    ...
    assetPnl[trade.symbol].pnl += trade.profit_loss || 0;
...
BEST TRADES
-----------
Best Trade: ${bestTrade ? `${bestTrade.symbol} - $${bestTrade.profit_loss?.toFixed(2)}` : 'N/A'}
Best Day: ${bestDay.date ? `${format(new Date(bestDay.date), 'MMM dd')} - $${bestDay.pnl.toFixed(2)}` : 'N/A'}

WORST TRADES
------------
Worst Trade: ${worstTrade ? `${worstTrade.symbol} - $${worstTrade.profit_loss?.toFixed(2)}` : 'N/A'}
