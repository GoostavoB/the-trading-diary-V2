...
const fragmentShader = `
  ...
  void main() {
    ...
    vec2 light2 = vec2(0.5 + cos(uTime * 0.12) * 0.4, 0.5 + sin(uTime * 0.08) * 0.4);
    ...
    vec3 accentColor = vec3(0.05, 0.08, 0.12);
    
    vec3 color = mix(baseColor, accentColor, noise + light);
    color *= uIntensity;
    
    // Film grain (2%)
    float grain = random(uv + uTime * 0.1) * 0.02;
    color += grain;
...
export const ShaderBackground = ({ intensity = 1.0 }: { intensity?: number }) => {
  // Check for reduced motion preference
  const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  if (prefersReducedMotion) {
    return (
      <div className="fixed inset-0 -z-10 bg-[#050608]" style={{ 
        backgroundImage: 'radial-gradient(circle at 50% 50%, rgba(10, 15, 20, 0.8) 0%, rgba(5, 6, 8, 1) 100%)'
      }} />
    );
  }
