    ...
    <div className="space-y-3">
      {sorted.length === 0 && (
        <p className="text-sm text-muted-foreground text-center py-4 rounded-xl border border-dashed border-border">
          Nenhum perfil salvo ainda. Crie um abaixo — ex: "Scalp" 2%, "Swing" 5%, "Alto Risco" 8%.
        </p>
      )}
      {sorted.map((p, i) => (
        <div key={p.id} className="flex items-center gap-2 bg-muted/30 rounded-xl p-3 border border-border">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-muted-foreground font-mono">{p.risk_pct}% de risco por trade</div>
          </div>
          <button
      ...
      <div className="rounded-xl border border-border p-3 space-y-2 bg-card">
        <Label className="text-xs text-muted-foreground uppercase tracking-wider">Novo perfil</Label>
...
function GordurinhaCard({
  ...
  if (isActive) {
    return (
      <div className="rounded-xl border border-apple-orange/30 bg-gradient-to-r from-apple-orange/10 to-apple-purple/10 p-4 flex items-center gap-3">
        ...
        <div>
          <div className="text-sm font-bold text-apple-orange">Meta batida — Gordurinha liberada!</div>
          <div className="text-xs text-muted-foreground">
            Capital de risco livre: <span className="font-mono font-semibold text-foreground">{formatAmount(amount)}</span>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="rounded-xl border border-dashed border-border bg-muted/10 p-4 flex items-center gap-3 opacity-70">
      ...
      <div>
        <div className="text-xs font-semibold text-muted-foreground">Gordurinha bloqueada</div>
        <div className="text-[11px] text-muted-foreground">Bata a meta mensal pra liberar risco extra com o lucro livre.</div>
      </div>
    </div>
  );
}
...
function MedalsBoard({ medals }: { medals: { id: string; medal: string; monthLabel: string }[] }) {
  return (
    <div className="space-y-2">
      <Label className="text-sm font-semibold">Quadro de Medalhas</Label>
      {medals.length > 0 ? (
      ...
      {medals.length === 0 && (
        <p className="text-[11px] text-muted-foreground text-center">Feche seu primeiro mês com meta batida pra começar a coleção.</p>
      )}
    </div>
  );
}
        ...
        <div className="p-6 space-y-7">
          ...
          {rc.loading ? (
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-base font-bold">Configurações de Risco</h3>
              ...
              <div className="space-y-3 rounded-xl border border-border p-4">
                <div className="flex items-center justify-between">
                  <Label className="text-sm font-semibold">Risco Piso (Modo Defesa)</Label>
                  <span className="text-sm font-mono font-bold text-apple-orange">{floorInput}%</span>
                </div>
                <p className="text-[11px] text-muted-foreground">Risco mínimo usado quando seu Win Rate está fraco.</p>
                <Slider value={[floorInput]} onValueChange={(v) => setFloorInput(v[0])} min={0} max={12} step={0.5} />
              </div>
              <div className="space-y-3 rounded-xl border border-border p-4">
                <div className="flex items-center justify-between">
                  <Label className="text-sm font-semibold">Risco Teto (Modo Sniper Elite)</Label>
                  <span className="text-sm font-mono font-bold text-apple-green">{ceilingInput}%</span>
                </div>
                <p className="text-[11px] text-muted-foreground">Risco máximo autorizado quando seu Win Rate está forte. Limite absoluto: 12%.</p>
              ...
              <Button className="w-full" onClick={saveSettings}>Salvar Configurações</Button>
              <div className="space-y-3 pt-2 border-t border-border">
                <Label className="text-sm font-semibold">Perfis de Risco</Label>
                <RiskProfileManager
                  profiles={profiles}
            ...
            <>
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-border bg-gradient-to-b from-muted/30 to-muted/10 p-6 text-center space-y-1.5">
                  <div className="text-xs font-mono text-muted-foreground uppercase tracking-wider">
                    {selectedProfile ? `Stop com o perfil "${selectedProfile.name}"` : 'Stop Autorizado Hoje'}
                  </div>
                  ...
                  {rc.tier !== 'red' && (
                    <p className="text-xs text-muted-foreground pt-1.5 flex items-center justify-center gap-1">
                      <Info className="h-3 w-3" /> Alvo 1 = Parcial + Stop no 0x0 (Break Even)
                    </p>
                  )}
                </div>
              </div>
              <div className="space-y-3">
                ...
                {profiles.length === 0 && (
                  ...
                  >
                    + Criar seu primeiro perfil de risco (Scalp, Swing, Alto Risco...)
                  </button>
                )}
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground font-medium">Capital de Trade Atual</span>
