...
const WinsByHourChartComponent = ({ trades }: WinsByHourChartProps) => {
  ...
  const { hourlyData, bestHour, worstHour, aiInsight } = useMemo(() => {
    ...
    const insight = hoursWithTrades.length > 0
      ? `Your best performance happens at ${best.hour} (avg ${formatNumber(best.totalPnL)} P&L, ${formatPercent(best.avgROI)} ROI). ${worst.totalPnL < 0
  ...
  return (
    <PremiumCard className="glass-card border-white/5">
